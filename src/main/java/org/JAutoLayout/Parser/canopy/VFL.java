...
public class VFL extends Grammar {
    public VFL(String input, Actions actions) {
        this.input = input;
    ...
    public static TreeNode parse(String input, Actions actions) throws ParseError {
        VFL parser = new VFL(input, actions);
        return parser.parse();
    }
    ...
    private TreeNode parse() throws ParseError {
        TreeNode tree = _read_visualFormatString();
        if (tree != FAILURE && offset == inputSize) {
            return tree;
        }
        if (expected.isEmpty()) {
            failure = offset;
            expected.add(new String[] { "VFL", "<EOF>" });
        }
